...
const addArticle =(obj)=>{
  return new Promise((resolve,reject)=>{
  const pointer = wx.Bmob.Pointer('categories');
  var poiID1 = pointer.set(obj.labels);
  const query = wx.Bmob.Query('articles')
  query.set('title',obj.title)
  query.set('author',obj.author)
  query.set('read_counts',0)
  query.set('like',0)
  query.set('category',poiID1)
  query.set('mdcontent',obj.gist.content)
  query.set('listPic',obj.main_pic)
  query.save().then(res=>{
    resolve({
      'result':true,
      'data':res
...
const editArticle =(obj)=>{
  return new Promise((resolve, reject) => {
  const query = wx.Bmob.Query('articles')
  query.get(obj.openid).then(res=>{
    res.set('listPic',obj.main_pic)
    res.set('mdcontent',obj.gist.content)
    res.set('title',obj.title)
    res.save()
    resolve({
      'result': true
    })
...
const likeAction = (id, action) => {
  return new Promise((resolve, reject) => {
    ...
    let uid = current.objectId
    const query = wx.Bmob.Query('like');
    const query1 = wx.Bmob.Query('articles');
    if (action == 'noLike') {
      query1.get(id).then(res => {
        res.set('like', (parseInt(res.like) - 1));
        res.save();
      })
      query.equalTo('article', '==', id);
      query.equalTo('user', '==', uid);
...
module.exports = {
  ...
  getArticleByUser:getArticleByUser,
  delActile:delActile,
